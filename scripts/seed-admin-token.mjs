...
const scope = process.env.WAZOO_ADMIN_SCOPE || "admin organizations.read organizations.write worlds.read worlds.write worlds.admin usage.read billing.read";
const name = process.env.WAZOO_ADMIN_TOKEN_NAME || "bootstrap-admin";

const client = createClient({ url, authToken });
await client.execute({
  sql: "INSERT INTO platform_api_tokens (uid, organization_uid, name, token_hash, kind, scope) VALUES (?, NULL, ?, ?, 'ADMIN', ?)",
