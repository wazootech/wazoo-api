...
export const organizations = new Hono<AppEnv>()
  .get("/organizations", async (c) => {
    ...
    if (isAdmin(c)) {
      ...
      if (email) {
        const user = await first<{ uid: string }>(
          database.prepare("SELECT uid FROM users WHERE email = ?").bind(email),
        );
        if (user) {
          rows = await all(
            database
              .prepare(
                "SELECT * FROM organizations WHERE owner_user_uid = ? AND state != 'DELETED' ORDER BY create_time DESC",
              )
              .bind(user.uid),
          );
        } else {
        ...
        rows = await all(
          database.prepare(
            "SELECT * FROM organizations WHERE state != 'DELETED' ORDER BY create_time DESC",
          ),
        );
      }
      ...
      rows = await all(
        database
          .prepare(
            "SELECT * FROM organizations WHERE uid = ? AND state != 'DELETED' ORDER BY create_time DESC",
          )
    ...
    return c.json({
      organizations: await Promise.all(
        rows.map((row) => organizationResource(c, row as OrganizationRow)),
      ),
    });
  })
  .post("/organizations", async (c) => {
    ...
    if (
      !organizationBody ||
      typeof organizationBody !== "object" ||
    ...
    const organization = {
      uid: `org_${id()}`,
      organizationId: requireResourceId(body, "organizationId"),
      displayName: requireString(
        organizationBody as Record<string, unknown>,
        "displayName",
      ),
      now: now(),
    };
    ...
    await db(c.env)
      .prepare(
        "INSERT INTO organizations (uid, organization_id, display_name, owner_user_uid, create_time, update_time) VALUES (?, ?, ?, ?, ?, ?)",
      )
      .bind(
        organization.uid,
    ...
    return c.json(
      {
        organization: await organizationResource(c, {
          ...
          create_time: organization.now,
          update_time: organization.now,
        }),
      },
      201,
    );
  ...
  .get("/organizations/:organizationId", async (c) => {
    ...
    const row = await first<OrganizationRow>(
      db(c.env)
        .prepare("SELECT * FROM organizations WHERE uid = ?")
        .bind(organization.uid),
    );
    return c.json({
      organization: row ? await organizationResource(c, row) : null,
    });
  })
  .patch("/organizations/:organizationId", async (c) => {
      ...
      return c.json(
        {
          error: {
            code: "INVALID_ARGUMENT",
            message: "state must be ACTIVE or SUSPENDED",
          },
        },
        400,
      );
      ...
      return c.json(
        {
          error: {
            ...
            message: "Organization state changes require admin",
          },
        },
        403,
      );
    ...
    await db(c.env)
      .prepare(
        "UPDATE organizations SET display_name = COALESCE(?, display_name), state = COALESCE(?, state), update_time = ? WHERE uid = ?",
      )
      .bind(
    ...
    if (nextState) {
      await recordAdminAudit(c, {
        action: "organizations.patch_state",
        targetResourceName: `organizations/${organization.organizationId}`,
      });
    }
    const row = await first<OrganizationRow>(
      db(c.env)
  ...
  .delete("/organizations/:organizationId", async (c) => {
    ...
    await db(c.env)
      .prepare(
        "UPDATE organizations SET state = 'DELETED', delete_time = ?, expire_time = ?, purge_status = 'pending', update_time = ? WHERE uid = ?",
      )
