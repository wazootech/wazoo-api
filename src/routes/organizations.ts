...
export const organizations = new Hono<AppEnv>()
  ...
  .patch("/organizations/:organizationId", async (c) => {
    ...
    await db(c.env).prepare("UPDATE organizations SET name = ?, updated_at = ? WHERE id = ?").bind(requireString(organizationBody as Record<string, unknown>, "displayName"), now(), organization.id).run();
    const row = await first<OrganizationRow>(db(c.env).prepare("SELECT * FROM organizations WHERE id = ?").bind(organization.id));
    return c.json({ organization: row ? organizationResource(row) : null });
  })
